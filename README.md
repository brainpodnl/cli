...
`agent start` always mints a new session and discards the previous one; it never merges, so running a workflow twice cannot leave the earlier deploy's steps showing under the new one. `agent step` requires `--label` the first time an id is recorded and updates it thereafter. `agent log` reads stdin and appends to `.brainpod/session.log`, tagging each line with `--stream` so one log can carry the build, the tests, and anything else in the order it happened. `image build` writes its own output there under `[build]`. Commands resolve the repository root rather than the working directory, so a command run from a subdirectory reaches the same console.
